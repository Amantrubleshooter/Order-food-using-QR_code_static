...
def admin_place_order(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            customer = User.objects.create_user(
                phone=phone,
                password=phone  # Simple password
            )
    ...
    recent_customers = User.objects.filter(
        phone__in=order.objects.values_list('phone', flat=True).distinct()
    ).distinct()[:10]
    
    # Get available tables
    available_tables = Table.objects.filter(is_occupied=False).order_by('table_number')
